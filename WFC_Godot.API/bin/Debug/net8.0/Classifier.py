...
def DataPreparation(image, labelsSides, labelsCorner, tileSize):
    labelsSides = np.array([np.array(xi) for xi in labelsSides])
    labelsCorner = np.array([np.array(xi) for xi in labelsCorner])
...
def LoadModelAndPeridct(images, tileSize):
    ...
    if K.image_data_format() == "channels_first":
        images = image.reshape(image.shape[0], tileSize,tileSize,image.shape[-1])
    else:
        images = image.reshape(image.shape[0], tileSize,tileSize,image.shape[-1])
    ...
    sidesDeencoded = onehot_to_categories(sidesPredicted,sidesCategories)
    cornersDeencoded = onehot_to_categories(cornersPredicted,cornerCategories)
    sidesDeencoded =sidesDeencoded.reshape(int(sidesDeencoded.shape[0]/4),4)
    cornersDeencoded =cornersDeencoded.reshape(int(cornersDeencoded.shape[0]/4),4)
    return [sidesDeencoded.tolist(),cornersDeencoded.tolist()]
